...
DomHandler.prototype._bubbling = function() {
  for (let i = this._tagStack.length - 1; i >= 0; i--) {
    if (trustTag[this._tagStack[i].name]) this._tagStack[i].continue = true
...
DomHandler.prototype.onopentag = function(name, attrs) {
  ...
  switch (name) {
    ...
    case 'img':
      if (attrs.width) {
        attrs.style =
          'width:' + attrs.width + (/[0-9]/.test(attrs.width[attrs.width.length - 1]) ? 'px' : '') + ';' + attrs.style
        delete attrs.width
      }
      if (attrs['data-src']) {
...
DomHandler.prototype.onclosetag = function(name) {
  ...
  if (element.children.length == 1 && element.name == 'div') {
    ...
    if (
      ...
      !(/margin/.test(element.attrs.style) && /margin/.test(child.attrs.style)) &&
      !/display/.test(element.attrs.style) &&
      !/display/.test(child.attrs.style) &&
      !(element.attrs.id && child.attrs.id) &&
      !(element.attrs.class && child.attrs.class)
    ) {
      if (/padding/.test(child.attrs.style)) child.attrs.style = ';box-sizing:border-box;' + child.attrs.style
  ...
  // 多层样式处理
  if (this._CssHandler.pop) this._CssHandler.pop(element)
}
